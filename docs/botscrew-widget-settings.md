...
interface GsbAppearance {
  ...
  messageStyle: 'classic' | 'minimal' | 'elevated' | 'squared'; // chat message bubble look
...
| `messageStyle` | enum | `classic` | **Animations & effects card.** Chat message bubble look: `classic` (solid fills + a real pointer tail), `minimal` (AI reply is plain text, only the guest gets a pill), `elevated` (both float as soft-shadowed cards), or `squared` (crisp sharp corners). Widget reads `body[data-msg-style]` |
