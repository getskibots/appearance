...
| `typingIndicator` | enum | `dots` | The "reply is being written" animation: `dots` (brand-tinted pulsing dots), `orb` (breathing brand orb, kin to the Voice Mode orb), or `label` ("{widgetName} is typing…" with animated ellipsis). Widget reads `body[data-typing-indicator]` + `data-typing-label` |
