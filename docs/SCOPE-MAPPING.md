...
| **6.3** Webcam — **single-cam v1** | ✅ ~90% | **Scope reduced to one cam** (multi-cam dropped). ✅ Built: paste-URL + **auto-detect kind** (still / MJPEG / YouTube / Vimeo / Roundshot / HLS / MP4 / unknown), **render across every format** (`<img>` / `<iframe>` / `<video>` / poster + "Open live cam ↗" / blocked notice), **status badge**, **location label**, **"last updated"** stamp, dark/night-image tolerance. This **exceeds** the original v1 (which only asked to *capture* iframe/HLS/MP4 URLs). ⬜ Dropped: multi-cam carousel (6s rotate, 15s pause, swipe, per-slide badges). Optional/not built: "Change type" override link. Offline still-image fallback intentionally skipped. Only the cross-cutting **per-bot save/load** of the URL remains. | [BUILD-STATUS §6.3](./BUILD-STATUS.md); `src/shared/webcam.js`, `src/shared/webcam-render.js` |
