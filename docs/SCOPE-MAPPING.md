...
| Req | Status | GSB note / answer | Where to read more |
|---|---|---|---|
| **6.1** Weather (Open-Meteo) | ✅ ~85% | ✅ Admin enters lat / long / elevation for **base + optional summit**; ✅ **live readings panel**; ✅ **chat preview**; ✅ launcher temp + condition icon; ✅ in-chat **6-cell card** (Winter/Summer, summer default). Vertical stack of source cards (**Open-Meteo live**, on by default), **Location Detail** validation panel, **⚡ Live preview** as-you-type, **JH coords + Summer** demo defaults (auto-fetched). Works today. | [BUILD-STATUS §6.1](./BUILD-STATUS.md); `weather.html` |
| **6.2** Weather (own endpoint) + SnoCountry | ⏸ parked ("Coming soon") | **Engines built but dormant** behind the placeholders: **Direct Feed** = multi-feed "build-your-own" (N endpoints/resort → auto-detect → map → combined preview), works in the dashboard, **not wired to the live widget** (needs a server-side proxy for CORS-blocked/secret feeds); **SnoCountry** = a working Live Status adapter (`snocountry.js`, CORS-open). `compose.js` turns the saved selection into the 6-cell card. **OpenSnow removed** (API access denied). Revive = un-park the two tiles. | [BUILD-STATUS §6.2](./BUILD-STATUS.md); **[RESORT-DIRECT-FEEDS.md](./RESORT-DIRECT-FEEDS.md)** |
| **6.3** Webcam — **multi-cam** | ✅ | **Multi-cam re-added.** ✅ Built: a **webcam list** (add/remove rows, URL + label each) with **auto-detect kind** (still / MJPEG / YouTube / Vimeo / Roundshot / HLS / MP4 / unknown) as a per-row badge, **render across every format** (`<img>` / `<iframe>` / `<video>` / poster + "Open live cam ↗" / blocked notice), and a **chat hero that rotates through 2+ cams every 6s** (`renderWebcamCarousel`) with per-cam labels (one cam = static). Old single `hero.webcam` migrates to `hero.webcams[]`. ⬜ Not built: drag-reorder (order = list order), 15s manual-pause / swipe, "Change type" override. Only the cross-cutting **per-bot save/load** remains. | [BUILD-STATUS §6.3](./BUILD-STATUS.md); `src/shared/webcam.js`, `src/shared/webcam-render.js` |
