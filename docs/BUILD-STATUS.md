...
- **Featured / hero image storage** — uploaded featured images are base64 `data:` blobs in the prototype; in production they must go to a **media endpoint** (we optimize client-side, you store + return a URL, we save only the URL in `gsbAppearance.hero`). Logos already use `POST /file/widgetLogo`; featured/hero assets need the same — ideally a generalized `/file/widgetAsset`. See [`botscrew-widget-settings.md`](./botscrew-widget-settings.md).
