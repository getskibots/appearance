...
export function applyWidgetConfig(config) {
  ...
  if (config.color && /^#?[a-f0-9]{6}$/i.test(config.color)) {
    ...
    setVar('--gsb-launcher-ring', lightBrand ? 'rgba(23,19,15,0.38)' : 'rgba(255,255,255,0.85)');
  }
  ...
  setVar('--gsb-avatar-size', config.avatarSize === 'small' ? '22px' : config.avatarSize === 'large' ? '36px' : '26px');
